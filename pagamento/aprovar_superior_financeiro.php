...
$sql = "SELECT user_id_user, favorecido_id_favorecido,id_solicitacao,reembolsavel
 FROM solicitacao WHERE id_solicitacao = $id_solicitacao";
...
if ($result->num_rows > 0) {
  ...
  while($row = $result->fetch_assoc()) {
    ...
    $sql2 = "UPDATE aprovacao SET aprovacao4 = 'Sim'
    WHERE solicitacao_id_solicitacao = $id_solicitacao";


    if ($link->query($sql2) === TRUE) {
  
        header('location:area_supervisor_financeiro.php');
